...
while($DistributionGroupList -eq 1)
{
    ...
    if($DistributionGroupMemAdd -eq "Y")
    {
        while($DistributionGroupUpdate -eq 1)
        {
            ...
            $AddAdditionalUsers = Read-Host -Prompt "Do we need to add additional users/contacts to this distribution group? (Y: Yes, N: No)"
    ...
    if($DistributionGroupMemRemove -eq "Y")
    {
        while($DistributionGroupUpdate -eq 1)
        {
            ...
            $RemoveAdditionalUsers = Read-Host -Prompt "Do we need to add additional users/contacts to this distribution group? (Y: Yes, N: No)"
